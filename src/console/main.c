...
int main(void)
{
    ...
    const UmiExperienceLayoutDefinition *layout =
        umi_database_studio_workspace_default();
    const UmiExperienceFeatureDefinition *next =
        umi_database_studio_workspace_next_feature();

    if (definition == NULL ||
        umi_database_studio_application_status(&status) != UMI_STATUS_OK ||
        layout == NULL) {
        fputs("Umicom Database Studio: Framework experience unavailable\n", stderr);
        return 1;
    }
