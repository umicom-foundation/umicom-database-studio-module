...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.database-studio") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_database_studio_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_database_studio_product_session_init(&session) ==
           UMI_STATUS_OK);
    ...
    assert(umi_database_studio_product_workspace_guide(&workspace_guide) ==
           UMI_STATUS_OK);
