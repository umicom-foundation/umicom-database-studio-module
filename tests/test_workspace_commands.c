...
int main(void)
{
    ...
    assert(umi_database_studio_runtime_init(&runtime) == UMI_STATUS_OK);
    assert(umi_database_studio_workspace_select_layout(
               &runtime, experience->default_layout_id) == UMI_STATUS_OK);
    assert(umi_database_studio_workspace_activate_panel(
               &runtime, experience->panels[0].panel_id) == UMI_STATUS_OK);
    commands = umi_database_studio_workspace_commands(&runtime);
    ...
    assert(umi_database_studio_workspace_commands(NULL) == NULL);
    return 0;
}
